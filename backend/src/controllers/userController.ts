...
export const editUser =  asyncHandler(async (req: Request, res: Response) =>{
  const { findEmail,firstName, lastName, email, password, userType } = req.body;
  console.log("heloooo")
  console.log("users"+User.toString)

  console.log("users"+User)

  let user = await User.findOne({ email:findEmail });
  if (!user) {
      res.status(404);
      throw new Error("User not found");
  }
  await User.findOneAndUpdate({ email:findEmail },{firstName:firstName,lastName:lastName,email:email,password:password,userType:userType});
  res.status(201).json({});
});